...
-(void)calibrate
{
    ...
    [self startTimerWithInterval:0.01];
...
-(void)setTreshold:(int)threshold
{
   ...
   // threshold = threshold*100;
    ...
    NSLog(@"_zeroTop_ %d", _zeroTop_);
    NSLog(@"_zeroBottom_ %d", _zeroBottom_);
...
- (void) melodySmart:(MelodySmart*)m didReceiveData:(NSData*)data
{
    ...
    }else if (outVal>_zeroTop_)
    {
        ...
        }else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
       //          NSLog(@"EXHALING - 1 ");
              //   NSLog(@"RANGE REDUCTION %f ", _rangeReduction_);
           //      NSLog(@"percent*_rangeReduction_ %f ", percent*_rangeReduction_);
